...
import { Edit, Mail, Phone, MapPin, Building2, Briefcase, Calendar, Badge, Clock, Shield } from 'lucide-react';
...
const ADMIN_PROFILE = {
  ...
  status: 'Active',
  permissions: ['All System Access', 'User Management', 'Audit Logs', 'Settings'],
};
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">System Administrator account details</p>
        </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`${roleColors.badge} ${roleColors.text} px-4 py-2 rounded-full font-semibold text-sm uppercase tracking-wide flex items-center gap-2`}>
                    <Shield size={14} />
                    {profileData.role}
                  </span>
            ...
            <div key={section.title} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                <div className="w-10 h-10 rounded-lg bg-red-50 flex items-center justify-center">
                  <SectionIcon size={20} className="text-red-600" />
                </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {profileData.permissions.map((permission) => (
            <div key={permission} className="flex items-center gap-3 p-3 bg-red-50 rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard icon={Badge} label="Employee ID" value={profileData.employeeId} color="red" />
        <StatCard icon={Building2} label="Department" value={profileData.department} color="pink" />
        <StatCard icon={Clock} label="Experience" value="3+ Years" color="emerald" />
      ...
      <div className="bg-red-50 rounded-2xl border border-red-100 p-6">
        ...
        <p className="text-sm text-red-700 mb-4">
          Last login: Recently • Manage security settings and system configurations
        </p>
