...
import { Edit, Mail, Phone, MapPin, Building2, Briefcase, Calendar, Badge, Clock, Users } from 'lucide-react';
...
const HR_PROFILE = {
  ...
  status: 'Active',
  responsibilities: ['Employee Recruitment', 'Leave Management', 'Performance Reviews', 'Employee Relations'],
};
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800">HR Profile</h2>
          <p className="text-sm text-slate-500 mt-1">HR Manager account details and responsibilities</p>
        </div>
            ...
            <div key={section.title} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                <div className="w-10 h-10 rounded-lg bg-purple-50 flex items-center justify-center">
                  <SectionIcon size={20} className="text-purple-600" />
                </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
          ...
          <h3 className="text-base font-semibold text-slate-800">Key Responsibilities</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {profileData.responsibilities.map((responsibility) => (
            <div key={responsibility} className="flex items-center gap-3 p-3 bg-purple-50 rounded-lg">
              <div className="w-2 h-2 bg-purple-500 rounded-full" />
              <span className="text-sm font-medium text-slate-700">{responsibility}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard icon={Badge} label="Employee ID" value={profileData.employeeId} color="purple" />
        <StatCard icon={Building2} label="Department" value={profileData.department} color="pink" />
        <StatCard icon={Clock} label="Experience" value="2+ Years" color="emerald" />
      ...
      <div className="bg-purple-50 rounded-2xl border border-purple-100 p-6">
        <h3 className="text-base font-semibold text-purple-900 mb-2">Account Actions</h3>
        <p className="text-sm text-purple-700 mb-4">
          Last login: Recently • Manage account settings and HR module configurations
        </p>
